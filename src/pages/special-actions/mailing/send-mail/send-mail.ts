...
export class SendMail extends TablePageBase implements ComponentCanDeactivate {
      ...
      (queryParams) => {
        if (queryParams.acc_id || queryParams.attach) {
          ...
          if (queryParams.content) {
            ...
            this.createMail(true).then((resp) => {
              this.saved = true;
  ...
  public removeMail(mail) {
    return this.alerts.confirmDeletion('Mail').subscribe((shouldDelete) => {
      if (shouldDelete) {
        this.mailing.remove(mail.id).subscribe(
          (resp) => {
  ...
  public getMail() {
    this.mailing.find(this.id, this.langMap[this.lang.abrev]).subscribe(
      (resp) => {
        this.mail = resp.data;
  ...
  public getStatusColor(status) {
    switch (status) {
      case MailingDeliveriesCrud.STATUS_ERRORED:
        return 'error';
      case MailingDeliveriesCrud.STATUS_SENT:
        return 'success';
      case MailingDeliveriesCrud.STATUS_SCHEDULED:
        return 'primary';
    }
  }
  ...
  public getMailStatusColor(status) {
    switch (status) {
      case MailingCrud.STATUS_CREATED:
        return 'info';
      case MailingCrud.STATUS_PROCESSED:
        return 'info';
      case MailingCrud.STATUS_SCHEDULED:
        return 'warning';
    }
  }
  ...
  public deleteAccount(item) {
    this.loading = true;
    this.mailDeliveries.remove(item.id).subscribe(
      (resp) => {
  ...
  public updateMail(data, message = 'Saved Mail Correctly') {
    return this.mailing
      .update(this.id, data, this.langMap[this.lang.abrev])
      .toPromise()
      .then((resp) => {
  ...
  public addAttachment(file, name = '') {
    ...
    this.mailing.addAttachment(this.id, attachments, 'en').subscribe(
      () => {
  ...
  public removeAttachment(name) {
    ...
    this.mailing.addAttachment(this.id, this.mail.attachments, this.langMap[this.lang.abrev]).subscribe(
      () => {
