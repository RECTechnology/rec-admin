...
*/
let defaultRes = ["15", "30", "60", "D", "W", "1M"];
...
Datafeeds.UDFCompatibleDatafeed.prototype.defaultConfiguration = function () {
	return {
		...
		supported_resolutions: defaultRes,
		supports_marks: false,
		supports_time: true,
		has_weekly_and_monthly: true.valueOf,
...
Datafeeds.UDFCompatibleDatafeed.prototype.getMarks = function (symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
	console.log("Get marks: " + 1511654400, rangeStart, rangeEnd, resolution)
	...
	if (window.getMarks) marks = window.getMarks();
	if (rangeStart < 1511654400 && rangeEnd > 1511654400);
	setTimeout(_ => {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function (symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
	if (this._configuration.supports_timescale_marks) {
		setTimeout(_ => {
			let marks = localStorage.getItem('trade-marks');
			if (marks) marks = JSON.parse(marks);
			onDataCallback([]);
		}, 0)
	}
};
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	setTimeout(x => {
		...
		let from = localStorage.getItem('trade-from') || 'BTC'
		let to = localStorage.getItem('trade-to') || 'EUR'
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	// console.log("resolution: ", resolution, symbolInfo)
	let resolutionMinutes = resolutions[resolution];
	let from = localStorage.getItem('trade-from') || 'BTC'
	let to = localStorage.getItem('trade-to') || 'EUR'


	let params = {
		interval: resolutionMinutes * 60,
		from: rangeStartDate,
		to: rangeEndDate + resolutionMinutes * 60
	}
	let paramsStr = ''
	for (var i = 0; i < Object.keys(params).length; ++i) {
		var key = Object.keys(params)[i];
	...
	$.ajax({
		type: 'GET',
		url: this._datafeedURL + '/exchange/v1/candlestick/' + from + '/' + to + paramsStr,
		contentType: 'application/json',
		accept: 'application/json'
	})
		.done(function (response) {
			response = parseJSONorNot(response);
			let bars = [];
			...
			onDataCallback(bars, { noData: null, nextTime: null });
		})
		.fail(function (arg) {
...
Datafeeds.SymbolsStorage.prototype._requestFullSymbolsList = function () {
	...
	for (var i = 0; i < this._exchangesList.length; ++i) {
		...
		try {
			this._datafeed._send('http://some-fix-url.com/exchange/v1/ticker/' + exchange).done((function (exchange) {
...
Datafeeds.SymbolsStorage.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	setTimeout(function () {
		if (!that._symbolsInfo.hasOwnProperty(symbolName)) {
			onResolveErrorCallback('invalid symbol 2');
		} else {
