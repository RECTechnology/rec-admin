...
export class CrudBaseService<T> extends BaseService2 {
  ...
  public static ROLE_PUBLIC: CrudRole = 'public';
  ...
  public list(query?: CrudQueryOptions,  lang: RecLang = REC_LANGS.ALL): Observable<any> {
    ...
    console.log("Im in list",url)

    return this.get(url, query, lang ? { 'Content-Language': lang, 'Accept-Language': lang } : null).pipe(
      this.itemMapper(),
    );
  }
  ...
  public find(id: any, lang: RecLang = REC_LANGS.EN): Observable<{ data: T;[key: string]: any } | any> {
    ...
    return this.get(url, null, lang ? { 'Content-Language': lang, 'Accept-Language': lang } : null).pipe(
      this.itemMapper(),
    );
  }
  ...
  public import(importOptions: { csv: string }) {
    ...
    return this.post(url, importOptions).pipe(this.itemMapper());
  }
    ...
    if (!this.mapItems) {
      ...
      return map((resp: any) => {
        if (resp && resp.data && resp.data.elements) {
          ...
          resp.data = resp.data.map((el) => this.mapper(el));
        } else {
