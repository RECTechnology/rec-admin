...
export class BaseService2 {
  ...
  public setFlag(name: string, on: boolean = true) {
    this.options.flags[name] = on;
  }

  public getFlag(name: string) {
    return this.options.flags[name] || false;
  }
  ...
  public getUrl(url?: any) {
    ...
    return [this.options.base_url, ...url].join('');
  }
  ...
  */
  ...
  public getAdditionalHeaders(): ({ [k: string]: string }) {
    ...
    if (this.getFlag('translateHeaders')) {
      // this.options.lang = UtilsService.getLocaleFromLang(this.us.lang);
      headers['Content-Language'] = this.options.lang || REC_LANGS.EN;
      headers['Accept-Language'] = this.options.lang || REC_LANGS.EN;
    }
    return headers;
  }
